...
- Amazon checker uses `HttpClient(rate_limit=5.0, rate_jitter=7.0)` for built-in inter-request delays, and **pins its TLS fingerprint** (`emulation=Emulation.Chrome145` + a `fingerprint_pool`). Amazon's WAF keys reputation on (IP, fingerprint); wafer's default advances each version, and a bare bump to Chrome147 got the prod VPS IP challenged on every request (checker went silently stale). Do NOT drop the pin. **After any wafer upgrade, re-verify prod Amazon via `/health/checkers` — a local run passes on a residential IP and won't catch a datacenter-IP block**
